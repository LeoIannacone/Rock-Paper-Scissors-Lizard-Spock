...
public class KP_GUI  extends Panel implements ActionListener, iKPIC_subscribeHandler2 
{
	...
	if(!this.xmlTools.isQueryConfirmed(ret))
	{
		return;
	}
	...
	else if( c == bSPARQLUpdate)
	{
		...
		taMemo.append("SPARQLUpdate confirmed:"+(this.xmlTools.isUpdateConfirmed(ret)?"YES":"NO")+"\n");
		taMemo.append("\n\n*** SSAP message status:"+xmlTools.getSSAPmsgStatus(ret));
	...
	else if( c == bRULE)
	{
		...
		taMemo.append("SIB MESSAGE:\n"+ret+"\n\nKP-CORE MESSAGE:"+kp.getErrMess()+"("+kp.getErrID()+")\n");
		taMemo.append("SPARQLUpdate Persistent confirmed:"+(this.xmlTools.isUpdateConfirmed(ret)?"YES":"NO")+"\n");
		taMemo.append("\n\n*** SSAP message status:"+xmlTools.getSSAPmsgStatus(ret));

		String updateID = this.xmlTools.getUpdateID(ret);
		ComboItemRenderable item = new ComboItemRenderable();
